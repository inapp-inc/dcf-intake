...
} from "../services/caseIdentityService.js";
...
function normalizeSearchQuery(q: string): string {
  ...
  return q.trim().replace(/^case\s*#?\s*/i, "").replace(/^IR-?/i, "IR-");

}



router.get("/cases/search", requireRoles("screener", "supervisor", "worker", "admin"), async (req, res, next) => {
  ...
  try {
    ...
    if (!raw) {
      ...
      res.status(400).json({ code: "BAD_REQUEST", message: "Query parameter q is required" });
    ...
    const { rows } = await query<{
      ...
      childMatchKey: string | null;

    }>(

      `SELECT DISTINCT c.id AS "caseId", c.external_id AS "externalId", c.child_display AS "childDisplay",

              c.status, c.updated_at AS "updatedAt", c.child_match_key AS "childMatchKey"

       FROM cases c

       LEFT JOIN form_51a_fields f

         ON f.case_id = c.id AND f.section_id = 'child' AND f.field_id = 'child_name'

       WHERE lower(c.external_id) LIKE $1

          OR lower(c.child_display) LIKE $2

          OR lower(f.value) LIKE $2
    ...
    // Prefer prefix matches on external_id when user typed a case number

    const sorted = [...rows].sort((a, b) => {

      const aPrefix = (a.externalId ?? "").toLowerCase().startsWith(term.toLowerCase()) ? 0 : 1;

      const bPrefix = (b.externalId ?? "").toLowerCase().startsWith(term.toLowerCase()) ? 0 : 1;
    ...
    const items = await Promise.all(
      ...
      sorted.map(async (r) => {
        ...
        if (r.childMatchKey) {
          ...
          const { rows: rel } = await query<{ count: number }>(

            `SELECT COUNT(*) AS count FROM cases WHERE child_match_key = $1 AND id != $2`,

            [r.childMatchKey, r.caseId],

          );

          relatedCount = Number(rel[0]?.count ?? 0);
        ...
        return {
          ...
          childDisplay: r.childDisplay ?? undefined,
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const { rows } = await query<{ child_match_key: string | null }>(

        "SELECT child_match_key FROM cases WHERE id = $1",
      ...
      const items = await findRelatedCases(caseId, rows[0].child_match_key);
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const relatedCases = await findRelatedCases(caseId, (c.child_match_key as string | null) ?? null);
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const { rows: caseRows } = await query<{ id: string; live_session_status: string }>(

        "SELECT id, live_session_status FROM cases WHERE id = $1",
      ...
      if (caseRows[0].live_session_status === "recording") {

        res.status(409).json({

          code: "CONFLICT",

          message: "Live demo call is active — end the live session before uploading a file",
