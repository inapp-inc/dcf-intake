# Screening & Initial Report Process (Demo)

Mermaid diagram for **screener Initial Report intake** through **supervisor screening**. Field-worker investigation and Field Reports are out of scope here.
...
flowchart TB
  ...
  subgraph SCREENER["① Screener — Initial Report intake · case status: in_progress"]
    direction TB
    A([New Initial Report case]) --> B[Upload hotline audio]
    B --> C{AI pipeline worker}

    subgraph PIPE["AI pipeline (after audio)"]
      direction TB
      T1[Transcribe · Whisper] --> T2[Clean transcript]
      T2 --> T3[NLP → populate Initial Report fields<br/>teal AI fields · checkpoint → ai_populating]
  ...
  H5 --> H6
  H6 --> H7[All required Initial Report fields filled<br/>sections: child · incident · reporter · household · filing]
  H7 --> H8[Complete Initial Report checkpoint]:::human
  ...
  subgraph SUPERVISOR["② Supervisor — screening · case status: pending_review"]
    ...
    Q --> R[Review Initial Report · transcript · triage · risk · background]
  ...
  subgraph CHECKPOINT["Initial Report checkpoint states"]
    direction LR
...
| `request_clarification` | `needs_clarification` | Returns to screener for more Initial Report work |

## Initial Report completion gates (before submit)
...
- **Triage:** keyword scan + LLM indicators (System Admin config) → `triage_flags` with `pending` → screener `confirm` / `dismiss`
