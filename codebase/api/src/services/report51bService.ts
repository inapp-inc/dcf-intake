...
[Worker signature and supervisor approval required]`;
  ...
  await query(
    `INSERT INTO report_51b_versions (case_id, content, status)
     VALUES ($1, $2::jsonb, 'draft')`,
    [caseId, JSON.stringify({ text: content, version })],
  );
...
export async function approveDraft(
  ...
  if (editedContent) {
    await query(
      `UPDATE report_51b_versions SET content = $2::jsonb, status = 'worker_approved', approved_by = $3
