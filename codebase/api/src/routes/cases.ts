...
router.get("/cases", requireRoles("screener", "supervisor", "worker", "admin"), async (req, res, next) => {
  try {
    ...
    let sql = `SELECT id AS "caseId", external_id AS "externalId", child_display AS "childDisplay", status,
              ...
              emergency, updated_at AS "updatedAt"
       FROM cases`;
    const params: string[] = [];
  ...
  try {
    ...
    await query(
      `INSERT INTO cases (id, external_id, emergency, area_office)
       VALUES ($1, $2, $3, $4)`,
      [id, externalId, emergency, req.user!.areaOffice ?? "Springfield"],
    );
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      await minio.putObject(config.minio.bucket, key, req.file.buffer, req.file.buffer.length, {
        "Content-Type": req.file.mimetype || "application/octet-stream",
      });

      await query(
        `INSERT INTO pipeline_state (case_id, current_stage, stages)
         VALUES ($1, 'transcription', '{"transcription":"pending"}'::jsonb)
         ON CONFLICT (case_id) DO UPDATE
           SET current_stage = 'transcription',
               stages = pipeline_state.stages || '{"transcription":"pending"}'::jsonb,
               updated_at = now()`,
        [caseId],
      );
